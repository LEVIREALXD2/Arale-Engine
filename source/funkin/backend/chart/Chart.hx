...
class Chart {
	...
	public static function loadEventsJson(songName:String) {
		...
		if (FileSystem.exists(path)) {
			try {
				...
				trace('Failed to load song event data for ${songName} ($path): ${Std.string(e)}');
			}
		}
		return data;
	}
	...
	public static function loadChartMeta(songName:String, difficulty:String = "normal", fromMods:Bool = true):ChartMetaData {
		...
		*/
		...
		CoolUtil.setFieldDefault(data, "name", songName);
		CoolUtil.setFieldDefault(data, "beatsPerMeasure", 4);
		CoolUtil.setFieldDefault(data, "stepsPerBeat", 4);
		CoolUtil.setFieldDefault(data, "needsVoices", true);
		CoolUtil.setFieldDefault(data, "icon", "face");
		CoolUtil.setFieldDefault(data, "difficulties", []);
	...
	public static function parse(songName:String, difficulty:String = "normal"):ChartData {
		...
		if (!FileSystem.exists(chartPath)) {
			trace('Chart for song ${songName} ($difficulty) at "$chartPath" was not found.');
			valid = false;
		}
		var data:Dynamic = null;
		if (valid) {
			try data = Json.parse(File.getContent(chartPath))
