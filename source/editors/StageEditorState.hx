...
class StageEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create()
	{
		...
		#if TOUCH_CONTROLS
		addMobilePad('LEFT_FULL', 'CHARACTER_EDITOR');
		addMobilePadCamera();
	...
	function addHelpScreen()
	{
		...
		var mobileTips = ["X/Y - Camera Zoom In/Out",
		...
		var desktopTips = ["E/Q - Camera Zoom In/Out",
			"J/K/L/I - Move Camera",
			"R - Reset Camera Zoom",
		...
		var str:Array<String> = #if TOUCH_CONTROLS (useDesktopThings ? desktopTips : mobileTips) #else desktopTips #end;
	...
	function screenUI()
	{
		...
		var tipTextText = 'Press ${#if TOUCH_CONTROLS 'F' + (useDesktopThings ? "1" : "") #else 'F1' #end} for Help';
		var tipText:FlxText = new FlxText(0, FlxG.height - 44, 300, tipTextText, 20);
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.keys.justPressed.ESCAPE #if TOUCH_CONTROLS || mobilePad.buttonB.justPressed #end)
		{
			if(!unsavedProgress)
			{
		...
		if(FlxG.keys.justPressed.W #if TOUCH_CONTROLS || mobilePad.buttonV.justPressed #end)
		{
		...
		else if(FlxG.keys.justPressed.S #if TOUCH_CONTROLS || mobilePad.buttonD.justPressed #end)
		{
			spriteListRadioGroup.checked = FlxMath.wrap(spriteListRadioGroup.checked + 1, 0, spriteListRadioGroup.labels.length-1);
			trace(spriteListRadioGroup.checked);
			checkUIOnObject();
		...
		if(#if FLX_DEBUG FlxG.keys.justPressed.F3 #else FlxG.keys.justPressed.F2 #end #if TOUCH_CONTROLS || (mobilePad.buttonS.justPressed && !mobilePad.buttonF.justPressed) #end)
		{
		...
		if(FlxG.keys.justPressed.F12 #if TOUCH_CONTROLS || (mobilePad.buttonS.justPressed && !mobilePad.buttonG.justPressed) #end)
			showSelectionQuad = !showSelectionQuad;
		
		var shiftMult:Float = 1;
		var ctrlMult:Float = 1;
		if(FlxG.keys.pressed.SHIFT #if TOUCH_CONTROLS || mobilePad.buttonC.pressed #end) shiftMult = 4;
		...
		if (FlxG.keys.pressed.J #if TOUCH_CONTROLS || (mobilePad.buttonLeft.pressed && mobilePad.buttonG.pressed) #end) camX -= camMove;
		if (FlxG.keys.pressed.K #if TOUCH_CONTROLS || (mobilePad.buttonDown.pressed && mobilePad.buttonG.pressed) #end) camY += camMove;
		if (FlxG.keys.pressed.L #if TOUCH_CONTROLS || (mobilePad.buttonRight.pressed && mobilePad.buttonG.pressed) #end) camX += camMove;
		if (FlxG.keys.pressed.I #if TOUCH_CONTROLS || (mobilePad.buttonUp.pressed && mobilePad.buttonG.pressed) #end) camY -= camMove;
		...
		if((FlxG.keys.justPressed.R #if TOUCH_CONTROLS || mobilePad.buttonZ.justPressed #end) && !FlxG.keys.pressed.CONTROL)
			FlxG.camera.zoom = stageJson.defaultZoom;
		else if ((FlxG.keys.pressed.E #if TOUCH_CONTROLS || mobilePad.buttonX.pressed #end) && FlxG.camera.zoom < maxZoom)
			FlxG.camera.zoom = Math.min(maxZoom, FlxG.camera.zoom + elapsed * FlxG.camera.zoom * shiftMult * ctrlMult);
		else if ((FlxG.keys.pressed.Q #if TOUCH_CONTROLS || mobilePad.buttonY.pressed #end) && FlxG.camera.zoom > minZoom)
		...
		var moveX:Float = 0;
		var moveY:Float = 0;
		#if TOUCH_CONTROLS if (!mobilePad.buttonG.pressed) { #end
		if (FlxG.keys.justPressed.LEFT #if TOUCH_CONTROLS || mobilePad.buttonLeft.justPressed #end) moveX -= 5 * shiftMult * ctrlMult;
		if (FlxG.keys.justPressed.RIGHT #if TOUCH_CONTROLS || mobilePad.buttonRight.justPressed #end) moveX += 5 * shiftMult * ctrlMult;
		if (FlxG.keys.justPressed.UP #if TOUCH_CONTROLS || mobilePad.buttonUp.justPressed #end) moveY -= 5 * shiftMult * ctrlMult;
		if (FlxG.keys.justPressed.DOWN #if TOUCH_CONTROLS || mobilePad.buttonDown.justPressed #end) moveY += 5 * shiftMult * ctrlMult;
		#if TOUCH_CONTROLS } #end
...
class StageEditorAnimationSubstate extends MusicBeatSubstate {
	...
	override function update(elapsed:Float)
	{
		...
		if(target.animations.length > 1)
		{
			...
			if((FlxG.keys.justPressed.W #if TOUCH_CONTROLS || mobilePad.buttonUp.justPressed #end) && (changedAnim = true)) curAnim--;
			else if((FlxG.keys.justPressed.S #if TOUCH_CONTROLS || mobilePad.buttonDown.justPressed #end) && (changedAnim = true)) curAnim++;
		...
		if(target.sprite.animation.curAnim != null)
		{
			...
			var moveKeysP = #if TOUCH_CONTROLS
			(useDesktopThings ? [FlxG.keys.justPressed.LEFT, FlxG.keys.justPressed.RIGHT, FlxG.keys.justPressed.UP, FlxG.keys.justPressed.DOWN
			] : [mobilePad.buttonLeft.justPressed, mobilePad.buttonRight.justPressed, mobilePad.buttonUp.justPressed, mobilePad.buttonDown.justPressed])
			#else [
			...
			#end;
			var moveKeys = #if TOUCH_CONTROLS 
			(useDesktopThings ? [FlxG.keys.pressed.LEFT, FlxG.keys.pressed.RIGHT, FlxG.keys.pressed.UP, FlxG.keys.pressed.DOWN
			] : [mobilePad.buttonLeft.pressed, mobilePad.buttonRight.pressed, mobilePad.buttonUp.pressed, mobilePad.buttonDown.pressed])
			#else
			[FlxG.keys.pressed.LEFT, FlxG.keys.pressed.RIGHT, FlxG.keys.pressed.UP, FlxG.keys.pressed.DOWN]
			#end;
			if(moveKeysP.contains(true) #if TOUCH_CONTROLS && !mobilePad.buttonG.pressed #end)
			{
			...
			if(moveKeys.contains(true) #if TOUCH_CONTROLS && !mobilePad.buttonG.pressed #end)
			{
			...
			if ((FlxG.keys.justPressed.R #if TOUCH_CONTROLS || mobilePad.buttonZ.justPressed #end) && (FlxG.keys.pressed.CONTROL #if TOUCH_CONTROLS || mobilePad.buttonC.pressed #end))
			{
		...
		if(FlxG.keys.justPressed.ESCAPE #if android || FlxG.android.justReleased.BACK #elseif TOUCH_CONTROLS || mobilePad.buttonB.justPressed #end)
		{
			persistentDraw = true;
			//controls.isInSubstate = false;
			close();
